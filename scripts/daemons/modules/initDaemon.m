...
/*
  ...
  +----------------------------------------------------------------------+
  | Last-Modified:2012-06-27 13:36:57                                    |
  +----------------------------------------------------------------------+
*/
...
if (!empty($GLOBALS['OPTIONS']['workers'])) {
    foreach ($GLOBALS['OPTIONS']['workers'] as $workerTitle=>$workerInfo) {
        ...
        $workScript=_DAEMON_ROOT._SUBDIR_WORKERS.'/'.$scriptFile;
...
if (!empty($GLOBALS['OPTIONS']['preload'])) {
    foreach ($GLOBALS['OPTIONS']['preload'] as $prePath=>$preBases) {
        ...
        foreach ($bases as $base) {
            $preFile=_DAEMON_ROOT._SUBDIR_WORKERS.'/'.$prePath.'/'.$base;
...
//files
$GLOBALS['_daemon']['pidFile']=_DAEMON_ROOT._SUBPATH_RUN.'/'.$GLOBALS['OPTIONS']['title'].'.pid';
_makeDir($GLOBALS['_daemon']['pidFile'],"0755",0,'f');
$GLOBALS['_daemon']['statusFile']=_DAEMON_ROOT._SUBPATH_RUN.'/'.$GLOBALS['OPTIONS']['title'].'.status';
_makeDir($GLOBALS['_daemon']['statusFile'],"0755",0,'f');
$GLOBALS['_daemon']['tmpDir']=_DAEMON_ROOT._SUBPATH_TMP;
