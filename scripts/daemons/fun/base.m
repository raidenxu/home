...
/*
  ...
  +----------------------------------------------------------------------+
  | Last-Modified:2013-05-13 18:20:41                                    |
  +----------------------------------------------------------------------+
*/
...
function sendMail($message,$subject,$addrs,$fromuser='Odin',$sysuser='Odin',$sysutil="/usr/sbin/sendmail -t",$contenttype="text/plain; charset=\"utf-8\""){
    $ret=false;
...
/* {{{ _checkInterval
 */
function _checkInterval(&$interval,&$lastRun,$defaultInterval) {
    $ret=false;
    ...
    do {
        ...
        $now=time();
        if ($lastRun<=0 || ($lastRun>0 && $now-$lastRun>=$interval)) {  //首次运行,或者超过间隔
            $lastRun=$now;
            if ($defaultInterval>0 && $interval!=$defaultInterval) { //这个代表不以默认间隔运行的情况只会发生一次
